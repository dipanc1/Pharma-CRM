...
-- Generated: 2025-11-29T05:55:48.130Z
...
-- Running balance per entry (computed via window function)
CREATE OR REPLACE VIEW public.ledger_entries_running AS
SELECT
  le.*,
  SUM(le.debit - le.credit) OVER (
    PARTITION BY le.doctor_id
    ORDER BY le.entry_date, le.created_at, le.id
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS running_balance
FROM public.ledger_entries le;

COMMENT ON VIEW public.ledger_entries_running IS 'Ledger entries with computed running balance per contact';
